...
public class ActionConfig {
	protected static final Supplier<ConfigGuiFactory> CONFIG_SCREEN_SUPPLIER = () -> new ConfigGuiFactory((minecraft, screen) -> new ConfigScreen(screen));
	...
	public static final class ConfigScreen extends Screen {
		private static final Component TITLE = new TranslatableComponent(InventoryActions.MODID + ".configScreen.title");
		private static final String ENABLE_ACTION_ICONS = InventoryActions.MODID + ".configScreen.enableActionIcons";
		private static final Component ENABLE_ACTION_ICONS_TOOLTIP = new TranslatableComponent(InventoryActions.MODID + ".configScreen.enableActionIcons.tooltip");
		...
		protected void init() {
			...
			CycleOption<Boolean> enableActionIcons = CycleOption.createOnOff(ENABLE_ACTION_ICONS, options -> ActionConfig.displayIconForValidActions.get(), (options, option, updatedValue) -> {
				ActionConfig.displayIconForValidActions.set(updatedValue);
				ActionConfig.save();
			}).setTooltip(mc -> value -> mc.font.split(ENABLE_ACTION_ICONS_TOOLTIP, 200));
