...
public class ItemMap {
	...
	public synchronized void write(FriendlyByteBuf buf) {
		FriendlyByteBuf.Writer<Holder<Item>> writer = (buff, holder) -> buff.writeResourceLocation(holder.unwrap().orThrow().location());

		buf.writeMap(this.map(), writer, writer);
	}

	public static ItemMap read(FriendlyByteBuf buf) {
		FriendlyByteBuf.Reader<Holder<Item>> reader = buff -> ForgeRegistries.ITEMS.getDelegateOrThrow(buff.readResourceLocation());
